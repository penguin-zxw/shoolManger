  ...
  <div class="container">
    ...
    >
      ...
      <el-table-column label="更新状态" align="center" width="100">
        <template slot-scope="scope">
          <span :style="scope.row.isend?'':'color:red'">{{scope.row.isend?'已完结':'连载中'}}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="sub_count"
      ...
      <el-table-column label="操作" align="center" width="300">
        <template slot-scope="scope">
          <router-link :to="'/course/column_detail/'+scope.row.id">
            <el-button size="mini" type="warning">目录</el-button>
          </router-link>
          
          <el-button style="margin-left:10px" size="mini" type="primary" @click="handleUpdate(scope.row)"
            >编辑</el-button
          >
          <el-button
            size="mini"
    ...
    >
      ...
      >
        ...
        <el-form-item label="课程内容"> </el-form-item>
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid)=>{
        if(valid){
          createColumn(this.listQuery).then((response) => {
            // console.log(response);
            if (response.code === 20000) {
    ...
    //编辑
    updateData() {
      updateColumn(this.listQuery).then((response) => {
        console.log(response);
        if (response.code === 20000) {
          this.dialogFormVisible = false;
          this.$notify({
